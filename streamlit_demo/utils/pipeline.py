...
PTH_CLEANSED = Path("data/cleansed")
PTH_FEATURED = Path("data/featured")
PTH_MAPPING = Path("data/mapping")
PTH_PREDICT = Path("data/predict")
PTH_RAW = Path("data/raw")
...
def shift_features(fixtures: pd.DataFrame, plays: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
